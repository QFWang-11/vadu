...
class OIMLoss(nn.Module):
    ...
    def forward(self, inputs, roi_label):
        ...
        mask=label<5554
        if mask.sum()==0:
            loss_oim = torch.tensor(0., device=inputs.device)
        else:
...
class MemContrast(nn.Module):
    """
    contrast_g2a=mem_contrast_g2a(ground_features,ground_pids,aerial_oim_lut,aerial_oim_cq)
    contrast_a2g=mem_contrast_a2g(aerial_features,aerial_pids,ground_oim_lut,ground_oim_cq)
    """
    def __init__(self,  num_features, temp=0.07):   #temp调参
        super(MemContrast, self).__init__()
        self.num_features = num_features
        self.temp=temp
    
    def forward(self, inputs, roi_label,lut,cq):
        # merge into one batch, background label = 0
        targets = torch.cat(roi_label)
        label = targets - 1  # background label = -1
        ...
        mask=label<5554
        if mask.sum()==0:
            loss_contrast = torch.tensor(0., device=inputs.device)
        else:
            inputs = inputs[inds.unsqueeze(1).expand_as(inputs)].view(-1, self.num_features)
            ...
            loss_contrast = F.cross_entropy(projected, label, ignore_index=5554)
