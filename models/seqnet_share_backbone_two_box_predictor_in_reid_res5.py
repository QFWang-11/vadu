...
class SeqNet(nn.Module):
    def __init__(self, cfg):
        ...
        faster_rcnn_predictor = FastRCNNPredictor(2048, 2)    # ResNet-34的输出通道数为512，类别数为2    
    ...
    def _rename_state_dict(self, state_dict):
        ...
        # new_key : old_key
        name_mapping = {
            'rpn_air.head.conv.0.0.weight': 'rpn.head.conv.weight',
            'rpn_air.head.conv.0.0.bias': 'rpn.head.conv.bias', 
            ...
            'rpn_ground.head.conv.0.0.weight': 'rpn.head.conv.weight', 
            'rpn_ground.head.conv.0.0.bias': 'rpn.head.conv.bias', 
...
class SeqRoIHeads(RoIHeads):
    def __init__(
        ...
        **kwargs
        ...
        self.box_predictor_gound=self.box_predictor
    ...
    def forward(self, features, image_shapes, features_air=None, features_ground=None, proposals_air=None, proposals_ground=None, targets_aerial=None, targets_ground=None, query_img_as_gallery=False):
        ...
        if combined_boxes and combined_boxes[0].shape[0]>0:     #不同虚拟环境版本对图像的处理有一些不同
            ...
            if n_air_boxes>0:
                box_features_a=box_features["feat_res5"][:n_air_boxes]
                box_regs_a=[self.box_predictor_air(box_features_a)]
            else:
                box_regs_a=[]
            if n_ground_boxes>0:
                box_features_g=box_features["feat_res5"][-n_ground_boxes:]
                box_regs_g=[self.box_predictor_ground(box_features_g)]
            else:
                box_regs_g=[]
            # box_regs = self.box_predictor(box_features["feat_res5"])
            box_regs = torch.cat(box_regs_a+box_regs_g,dim=0)
            assert box_regs.shape[0] == box_features.shape[0], "Invalid seperation of boxes !"
