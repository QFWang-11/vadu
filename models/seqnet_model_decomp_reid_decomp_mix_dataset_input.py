...
from models.resnet import build_resnet, build_resnet_last_stride1
...
class SeqNet(nn.Module):
    def __init__(self, cfg):
        ...
        backbone, box_head = build_resnet(name="resnet34", pretrained=True)
        #backbone, box_head, reid_head = build_resnet_last_stride1(name="resnet34", pretrained=True)  #返回两种res5，一个有2倍降采样的给检测，一个没有降采样的给reid
    ...
    def forward(self, images_aerial=None, images_ground=None, targets_aerial=None, targets_ground=None, query_img_as_gallery=False):
        """
        Args:
            images_aerial: Aerial view images (List[Tensor] or None)
            images_ground: Ground view images (List[Tensor] or None)
            targets_aerial: Aerial view targets (List[Dict] or None)
            targets_ground: Ground view targets (List[Dict] or None)
            query_img_as_gallery: Set to True to detect all people in the query image (for inference only).
        """
        # 输入检查：确保至少一个视角的图像输入非None
        if images_aerial is None and images_ground is None:
            raise ValueError("At least one of images_aerial or images_ground must be provided.")

        # 如果不是训练模式，调用inference方法
        if not self.training:
            if images_aerial is not None and images_ground is None:
                ...
                raise ValueError("Inference mode only supports single-view input (either aerial or ground).")
        ...
        if images_aerial is not None:
            ...
            images_aerial, targets_aerial = self.transform(images_aerial, targets_aerial)
            # 通过 backbone 获取特征
            features_aerial = self.backbone_aerial(images_aerial.tensors)
            features_aerial_dict = OrderedDict([("feat_res4", features_aerial["feat_res4"])])
            # 通过 RPN 获取 proposals 和损失
            proposals_aerial, proposal_losses_aerial = self.rpn_air(images_aerial, features_aerial_dict, targets_aerial)
            # 通过 RoI Heads 获取检测损失
            _, detector_losses_aerial = self.roi_heads(
                features_aerial_dict, images_aerial.image_sizes, features_aerial_dict, None,
                proposals_aerial, None, targets_aerial, None
            )
            # 重命名 RPN 损失
            proposal_losses_aerial["loss_rpn_reg"] = proposal_losses_aerial.pop("loss_rpn_box_reg", torch.tensor(0.0, device=images_aerial.tensors.device))
            proposal_losses_aerial["loss_rpn_cls"] = proposal_losses_aerial.pop("loss_objectness", torch.tensor(0.0, device=images_aerial.tensors.device))
            # 更新总损失
            losses.update({f"aerial_{k}": v for k, v in proposal_losses_aerial.items()})
            losses.update({f"aerial_{k}": v for k, v in detector_losses_aerial.items()})

        # 处理地面视角
        if images_ground is not None:
            # Transform images and targets
            images_ground, targets_ground = self.transform(images_ground, targets_ground)
            # 通过 backbone 获取特征
            features_ground = self.backbone_ground(images_ground.tensors)
            features_ground_dict = OrderedDict([("feat_res4", features_ground["feat_res4"])])
            # 通过 RPN 获取 proposals 和损失
            proposals_ground, proposal_losses_ground = self.rpn_ground(images_ground, features_ground_dict, targets_ground)
            # 通过 RoI Heads 获取检测损失
            _, detector_losses_ground = self.roi_heads(
                features_ground_dict, images_ground.image_sizes, None, features_ground_dict,
                None, proposals_ground, None, targets_ground
            )
            # 重命名 RPN 损失
            proposal_losses_ground["loss_rpn_reg"] = proposal_losses_ground.pop("loss_rpn_box_reg", torch.tensor(0.0, device=images_ground.tensors.device))
            proposal_losses_ground["loss_rpn_cls"] = proposal_losses_ground.pop("loss_objectness", torch.tensor(0.0, device=images_ground.tensors.device))
            # 更新总损失
            losses.update({f"ground_{k}": v for k, v in proposal_losses_ground.items()})
            losses.update({f"ground_{k}": v for k, v in detector_losses_ground.items()})

        # 如果两个视角都有，合并损失（这里简单相加，也可以按需加权）
        if images_aerial is not None and images_ground is not None:
            combined_losses = {}
            for key in losses.keys():
                if key.startswith("aerial_"):
                    base_key = key.replace("aerial_", "")
                    if f"ground_{base_key}" in losses:
                        combined_losses[base_key] = 0.5 * losses[f"aerial_{base_key}"] + 0.5 * losses[f"ground_{base_key}"]
                    else:
                        combined_losses[base_key] = losses[f"aerial_{base_key}"]
                elif key.startswith("ground_") and key.replace("ground_", "") not in combined_losses:
                    combined_losses[key.replace("ground_", "")] = losses[key]
            losses = combined_losses
        elif images_aerial is not None:
            losses = {k.replace("aerial_", ""): v for k, v in losses.items()}
        elif images_ground is not None:
            losses = {k.replace("ground_", ""): v for k, v in losses.items()}

        # 应用损失权重
        losses["loss_rpn_reg"] = losses.get("loss_rpn_reg", torch.tensor(0.0, device=images_aerial.tensors.device if images_aerial is not None else images_ground.tensors.device)) * self.lw_rpn_reg
        losses["loss_rpn_cls"] = losses.get("loss_rpn_cls", torch.tensor(0.0, device=images_aerial.tensors.device if images_aerial is not None else images_ground.tensors.device)) * self.lw_rpn_cls
        losses["loss_proposal_reg"] = losses.get("loss_proposal_reg", torch.tensor(0.0, device=images_aerial.tensors.device if images_aerial is not None else images_ground.tensors.device)) * self.lw_proposal_reg
        losses["loss_proposal_cls"] = losses.get("loss_proposal_cls", torch.tensor(0.0, device=images_aerial.tensors.device if images_aerial is not None else images_ground.tensors.device)) * self.lw_proposal_cls
        losses["loss_box_reg"] = losses.get("loss_box_reg", torch.tensor(0.0, device=images_aerial.tensors.device if images_aerial is not None else images_ground.tensors.device)) * self.lw_box_reg
        losses["loss_box_cls"] = losses.get("loss_box_cls", torch.tensor(0.0, device=images_aerial.tensors.device if images_aerial is not None else images_ground.tensors.device)) * self.lw_box_cls
        losses["loss_box_reid"] = losses.get("loss_box_reid", torch.tensor(0.0, device=images_aerial.tensors.device if images_aerial is not None else images_ground.tensors.device)) * self.lw_box_reid
...
class SeqRoIHeads(RoIHeads):
    def __init__(
        ...
        **kwargs
        ...
        # Re-ID head 共享
        self.embedding_head = NormAwareEmbedding()
        self.reid_loss = OIMLoss(256, num_pids, num_cq_size, oim_momentum, oim_scalar)        #有实验batchsize5_5 output512维度
    ...
    def forward(self, features, image_shapes, features_air=None, features_ground=None, proposals_air=None, proposals_ground=None, targets_aerial=None, targets_ground=None, query_img_as_gallery=False):
        ...
        if combined_boxes and combined_boxes[0].shape[0]>0:     #不同虚拟环境版本对图像的处理有一些不同
            ...
            #torch.Size([768, 256, 16, 8]) / torch.Size([768, 256, 14, 14])
            box_features = self.reid_head(box_features)
            #print(box_features["feat_res5"].shape)
            box_regs = self.box_predictor(box_features["feat_res5"])
            box_embeddings, box_cls_scores = self.embedding_head(box_features)
            if box_cls_scores.dim() == 0:
                box_cls_scores = box_cls_scores.unsqueeze(0)
        ...
        if self.training:
            ...
            loss_box_reid = 0.5 * loss_box_reid + 0.25 * loss_box_reid_air + 0.25 * loss_box_reid_ground
            #loss_box_reid = loss_box_reid + 0.25 * loss_box_reid_air + 0.25 * loss_box_reid_ground
            #loss_box_reid = (1/3) * loss_box_reid + (1/3) * loss_box_reid_air + (1/3) * loss_box_reid_ground
            #loss_box_reid = 0.5 * loss_box_reid + 0.5 * loss_box_reid_air + 0.5 * loss_box_reid_ground
...
class NormAwareEmbedding(nn.Module):
    ...
    def __init__(self, featmap_names=["feat_res4", "feat_res5"], in_channels=[256, 512], dim=256):   #有实验batchsize5_5 output512维度
