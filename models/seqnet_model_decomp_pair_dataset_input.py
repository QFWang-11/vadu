...
from models.triplet import TripletLoss
...
class SeqNet(nn.Module):
    ...
    def forward(self, images_aerial=None, images_ground=None, targets_aerial=None, targets_ground=None, query_img_as_gallery=False):
        ...
        # Compute SDM Loss
        #sdm_loss = compute_sdm(features_aerial["feat_res4"], features_ground["feat_res4"], 
        #                       pid=pid, logit_scale=self.logit_scale, image_id=image_id)
        # 检测部分的loss
...
class SeqRoIHeads(RoIHeads):
    ...
    def forward(self, features, image_shapes, features_air=None, features_ground=None, proposals_air=None, proposals_ground=None, targets_aerial=None, targets_ground=None, query_img_as_gallery=False):
        ...
        if self.training:
            ...
            all_pid_labels=torch.cat(combined_pid_labels)
            n_air_boxes=0+sum(boxes_i.shape[0] for boxes_i in boxes_air)
            n_ground_boxes=0+sum(boxes_i.shape[0] for boxes_i in boxes_ground)
            box_embeddings_air=box_embeddings[:n_air_boxes]
            pid_labels_air=all_pid_labels[:n_air_boxes]
            box_embeddings_ground=box_embeddings[-n_ground_boxes:]
            pid_labels_ground=all_pid_labels[-n_ground_boxes:]
            loss_sdm=compute_sdm_with_unlabeled(box_embeddings_air,box_embeddings_ground,pid_labels_air,pid_labels_ground)
            losses_baseline_detection.update(loss_box_reid=loss_box_reid,loss_sdm=loss_sdm)
...
#对比loss
# image_features[batcisize, feature_dim]   text_features[batcisize, feature_dim], pid[batcisize]
# loss如果不怎么降低了，就调整temp系数，尝试0.05， 0.07
def compute_sdm(features1, features2, pid1,pid2, temp=0.07, factor=0.3, epsilon=1e-8):
    """
    Similarity Distribution Matching
    """
    mask1=torch.logical_and(pid1<5554,pid1>=0)
    mask2=torch.logical_and(pid2<5554,pid2>=0)
    ...
    if mask1.sum()==0 or mask2.sum()==0 or len(set(pid1.cpu().numpy().tolist())-set(pid2.cpu().numpy().tolist()))==0:
        return torch.tensor(0.,device=features1.device)

    labels = (pid1.view(-1,1)==pid2.view(1,-1)).float()

    mask1_has_pos=labels.sum(dim=-1)>0
    mask2_has_pos=labels.t().sum(dim=-1)>0

    feat1_norm = features1 / features1.norm(dim=1, keepdim=True)
    feat2_norm = features2 / features2.norm(dim=1, keepdim=True)

    feat21_cosine_theta = feat2_norm @ feat1_norm.t()/temp
    feat12_cosine_theta = feat21_cosine_theta.t()/temp

    feat21_cosine_theta=feat21_cosine_theta[mask2_has_pos]
    feat12_cosine_theta=feat12_cosine_theta[mask1_has_pos]

    # normalize the true matching distribution
    labels_distribute12 = labels[mask1_has_pos] / labels[mask1_has_pos].sum(dim=1,keepdim=True)
    labels_distribute21 = labels.t()[mask2_has_pos] / labels.t()[mask2_has_pos].sum(dim=1,keepdim=True)

    """
    feat12_pred = F.softmax(feat12_cosine_theta, dim=1)
    feat12_loss = feat12_pred* (F.log_softmax(feat12_cosine_theta, dim=1) - torch.log(labels_distribute12 + epsilon))
    feat21_pred = F.softmax(feat21_cosine_theta, dim=1)
    feat21_loss = feat21_pred * (F.log_softmax(feat21_cosine_theta, dim=1) - torch.log(labels_distribute21 + epsilon))
    """

    feat12_loss=stable_kl(feat12_cosine_theta,labels_distribute12 + epsilon)
    feat21_loss=stable_kl(feat21_cosine_theta,labels_distribute21 + epsilon)

    loss = feat12_loss + feat21_loss

    #loss = torch.mean(torch.sum(feat12_loss, dim=1)) + torch.mean(torch.sum(feat21_loss, dim=1))

    return loss

def compute_sdm_with_unlabeled(features1, features2, pid1,pid2, temp=0.07, factor=0.3, epsilon=1e-8):
    mask1=pid1>=0
    mask2=pid2>=0
    ...
    mask1_has_pos=torch.logical_and(labels.sum(dim=-1)>0,pid1<5554)
    mask2_has_pos=torch.logical_and(labels.t().sum(dim=-1)>0,pid2<5554)
...
def stable_kl(logits_ture,prob_pred):
    logits_true_max= torch.max(logits_ture,dim=1,keepdim=True)[0].detach()
    logits_ture=logits_ture-logits_true_max
    sum_exp_true=torch.sum(logits_ture.exp(),dim=1,keepdim=True)

    kld=logits_ture.exp()/sum_exp_true*(logits_ture-sum_exp_true.log()-prob_pred.log())

    return kld.sum()/logits_ture.shape[0]