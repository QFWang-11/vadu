...
from datasets.build_1 import build_train_loader, build_test_loader    #加载混合数据集
from datasets.build import build_train_loader_aerial, build_train_loader_ground, build_test_loader_aerial, build_test_loader_ground, build_dataset  #加载单一视角数据集
from defaults import get_default_cfg
from engine import train_one_epoch, evaluate_performance_Re_ID

#from models.seqnet_v21 import SeqNet    #3个re-ID loss
#from models.seqnet_v21_3teacher import SeqNet
from models.seqnet_v22_3teacher import SeqNet
...
def main(args):
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCHS):
        ...
        train_one_epoch(cfg, model, optimizer, train_loader_aerial, train_loader_ground, device, epoch, tfboard)
        lr_scheduler.step()
        #先保存，再评估
        if (epoch + 1) % cfg.CKPT_PERIOD == 0 or epoch == cfg.SOLVER.MAX_EPOCHS - 1:
            save_on_master(
