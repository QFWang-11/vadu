...
class SeqNet(nn.Module):
    def __init__(self, cfg):
        ...
        backbone, box_head, _ = build_resnet(name="resnet34", pretrained=True)
...
class SeqRoIHeads(RoIHeads):
    ...
    def forward(self, features, image_shapes, features_air=None, features_ground=None, proposals_air=None, proposals_ground=None, targets_aerial=None, targets_ground=None, query_img_as_gallery=False):
        ...
        if combined_boxes and combined_boxes[0].shape[0]>0:     #不同虚拟环境版本对图像的处理有一些不同
            ...
            teacher_input = box_features.clone().detach()  # 为 teacher_head 准备输入
        ...
        if self.training:
            ...
            loss_box_reid, feat_det, label_det, logits_det = self.reid_loss(box_embeddings, combined_pid_labels)
            ...
            loss_box_reid_air, feat_det_air, label_det_air, logits_det_air = self.reid_loss_air(box_embeddings_air, box_pid_labels_air)
            loss_box_reid_ground, feat_det_ground, label_det_ground, logits_det_ground = self.reid_loss_ground(box_embeddings_ground, box_pid_labels_ground)
            ...
            if self.lw_oim_loss2 < 1e-5:
                with torch.no_grad():
                    oim_loss2, feat_ins, label_ins, logits_ins = self.teacher_head(teacher_input, combined_pid_labels)  #teacher head也是混合输出
            else:
                oim_loss2, feat_ins, label_ins, logits_ins = self.teacher_head(teacher_input, combined_pid_labels)
                losses_baseline_detection.update(oim_loss2=oim_loss2)
            
            # 分割 teacher 的输出以匹配 air 和 ground 的样本数
            total_air_samples = feat_det_air.shape[0]  # 40
            total_ground_samples = feat_det_ground.shape[0]  # 11
            # 分割 teacher 的特征和 logits
            feat_ins_air = feat_ins[:total_air_samples]  # [40, 256]
            feat_ins_ground = feat_ins[total_air_samples:total_air_samples + total_ground_samples]  # [11, 256]
            logits_ins_air = logits_ins[:total_air_samples]  # [40, 5532]
            logits_ins_ground = logits_ins[total_air_samples:total_air_samples + total_ground_samples]  # [11, 5532]
            #print('feat_ins:', feat_ins.shape)  #feat_ins: torch.Size([51, 256])
            #print('feat_ins_air:', feat_ins_air.shape) #feat_ins_air: torch.Size([40, 256])
            #print('feat_ins_ground:', feat_ins_ground.shape) #feat_ins_ground: torch.Size([11, 256])
            #print('feat_det:', feat_det.shape)  #feat_det: torch.Size([51, 256])
            #print('feat_det_air:', feat_det_air.shape) #feat_det_air: torch.Size([40, 256])
            #print('feat_det_ground:', feat_det_ground.shape) #feat_det_ground: torch.Size([11, 256])
            #print('label_ins:', label_ins.shape) #label_ins: torch.Size([51])
            #print('label_det:', label_det.shape) #label_det: torch.Size([51])
            #print('label_det_air:', label_det_air.shape) #label_det_air: torch.Size([40])
            #print('label_det_ground:', label_det_ground.shape) #label_det_ground: torch.Size([11])
            #print('logits_ins:', logits_ins.shape) #logits_ins: torch.Size([51, 5532])
            #print('logits_det:', logits_det.shape) #logits_det: torch.Size([51, 5532])
            #print('logits_det_air:', logits_det_air.shape) #logits_det_air: torch.Size([40, 5532])
            # 添加 KD 损失, 只和混合reid计算kl损失,3种都有
            if self.lw_kdfeat > 0:
                kd_feat = self.kd_feat_loss(feat_ins, feat_det)
                kd_feat_air = self.kd_feat_loss(feat_ins_air, feat_det_air)
                kd_feat_ground = self.kd_feat_loss(feat_ins_ground, feat_det_ground)
                kd_feat = 0.5*kd_feat + 0.25*kd_feat_air + 0.25*kd_feat_ground
                losses_baseline_detection.update(kd_feat=kd_feat)
                #losses_baseline_detection.update(kd_feat_air=kd_feat_air)
                #losses_baseline_detection.update(kd_feat_ground=kd_feat_ground)
            if self.lw_kd_sim_kl > 0:
                kd_sim_kl = self.kd_sim_kl(feat_ins, feat_det, self.tempsimkl, self.simkl_bidir)
                kd_sim_kl_air = self.kd_sim_kl(feat_ins_air, feat_det_air, self.tempsimkl, self.simkl_bidir)
                kd_sim_kl_ground = self.kd_sim_kl(feat_ins_ground, feat_det_ground, self.tempsimkl, self.simkl_bidir)
                kd_sim_kl = 0.5*kd_sim_kl + 0.25*kd_sim_kl_air + 0.25*kd_sim_kl_ground
                losses_baseline_detection.update(kd_sim_kl=kd_sim_kl * self.lw_kd_sim_kl)
                #losses_baseline_detection.update(kd_sim_kl_air=kd_sim_kl_air * self.lw_kd_sim_kl)
                #losses_baseline_detection.update(kd_sim_kl_ground=kd_sim_kl_ground * self.lw_kd_sim_kl)
            if self.lw_kd_pkt > 0:
                kd_pkt = self.kd_pkt(feat_ins, feat_det)
                kd_pkt_air = self.kd_pkt(feat_ins_air, feat_det_air)
                kd_pkt_ground = self.kd_pkt(feat_ins_ground, feat_det_ground)
                kd_pkt = 0.5*kd_pkt + 0.25*kd_pkt_air + 0.25*kd_pkt_ground
                losses_baseline_detection.update(kd_pkt=kd_pkt * self.lw_kd_pkt)
                #losses_baseline_detection.update(kd_pkt_air=kd_pkt_air * self.lw_kd_pkt)
                #losses_baseline_detection.update(kd_pkt_ground=kd_pkt_ground * self.lw_kd_pkt)
            if self.lw_kdlogits > 0:
                kd_logits = self.kd_logits_loss(logits_det, logits_ins)
                kd_logits_air = self.kd_logits_loss(logits_det_air, logits_ins_air)
                kd_logits_ground = self.kd_logits_loss(logits_det_ground, logits_ins_ground)
                kd_logits = 0.5*kd_logits + 0.25*kd_logits_air + 0.25*kd_logits_ground
                losses_baseline_detection.update(kd_logits=kd_logits)
                #losses_baseline_detection.update(kd_logits_air=kd_logits_air)
                #losses_baseline_detection.update(kd_logits_ground=kd_logits_ground)
