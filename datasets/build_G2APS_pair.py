...
from .G2APS_pair import CUHKSYSUPaired

'''
#导入混合视角数据集，就是原始的dataloader方案
...
def build_dataset(dataset_name, root, transforms, split, verbose=True):
    if dataset_name == "CUHK-SYSU":
        dataset = CUHKSYSUPaired(root, transforms, split)
...
def uni_collate_fn(batch):
    imgs,targets=[],[]
    for pair in batch:
        imgs.extend([pair[0][0],pair[1][0]])
        targets.extend([pair[0][1],pair[1][1]])
    return imgs,targets
